...
module.exports = function() {
	return function(req, res, next) {
		if(!req.soajs) { throw new TypeError('soajs mw is not started'); }

		if(!req.soajs.controller) { req.soajs.controller = {}; }
...
function extractBuildParameters(req, service, url) {
	...
	if(service && req.soajs.registry && req.soajs.registry.services && req.soajs.registry.services[service] && req.soajs.registry.services[service].port && req.soajs.registry.services[service].host) {
		servParameters = {};
		...
		servParameters.url = url.substring(service.length + 1);
...
function redirectToService(req, res, body) {
	...
	var requestOptions = {
		'method': req.method,
		'uri':     'http://' + restServiceParams.host + ':' + restServiceParams.port + restServiceParams.url,
		'timeout': 1000 * 3600,
...
function isRequestAuthorized(req, requestOptions, body) {
	...
	cookies.some(function(cookie, idx, arr) {
		if(cookie.indexOf(req.soajs.registry.serviceConfig.session.name)!== -1) { return true; }
	});
